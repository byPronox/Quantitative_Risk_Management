...
# Simple connection pool to avoid constant connect/disconnect
_connection_cache = {}
_connection_refs = weakref.WeakValueDictionary()

@asynccontextmanager
async def get_rabbitmq_connection():
    """Context manager for RabbitMQ connections with basic pooling"""
    manager = None
    try:
...
class RabbitMQManager:
    ...
    async def get_queue_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get actual queue info if possible
            message_count = 0
            consumer_count = 0
            
            if self.queue and self.channel:
                try:
                    # Try to get queue info via channel inspect
                    ...
                    # Fallback if passive declaration fails
                    message_count = 0
                    consumer_count = 0
            
            # Return enhanced queue info
            return {
                "connected": True,
                ...
                "keywords": list(set(r.get("keyword", "") for r in _job_results_store.values() if r.get("keyword"))),
                "pending": message_count,
                "processing": len([r for r in _job_results_store.values() if r.get("status") == "processing"]),
                "completed": len([r for r in _job_results_store.values() if r.get("status") == "completed"]),
                "failed": len([r for r in _job_results_store.values() if r.get("status") == "failed"]),
                "status": "connected"
            }
