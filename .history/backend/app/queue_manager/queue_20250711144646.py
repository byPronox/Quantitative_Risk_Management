...
class RabbitMQManager:
    ...
    async def get_job_metadata(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # to store job metadata in a database or Redis
            # For now, we'll return some basic info if we can parse the job_id
            
            if job_id:
                # Try to extract keyword from job context or use default
                return {
                    "job_id": job_id,
                    "keyword": "vulnerability",  # Default keyword for NVD search
                    "status": "processing"
                }
