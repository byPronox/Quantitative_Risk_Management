...
async def add_to_queue(request: Dict[str, Any]):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"RabbitMQ queue failed, using mock job: {e}")
            job_id = f"mock-job-{keyword}-{asyncio.get_event_loop().time()}"
            status = "queued_mock"
...
async def get_nvd_vulnerabilities(keywordSearch: str | None = None, **params):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code != 200:
                logger.error(f"Kong NVD API error: {response.status_code} - {response.text}")
                raise HTTPException(
        ...
        logger.error(f"Error calling Kong NVD API: {e}")
        raise HTTPException(status_code=503, detail="NVD API service unavailable")