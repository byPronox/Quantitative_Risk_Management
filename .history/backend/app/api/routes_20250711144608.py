...
async def get_job_results(job_id: str):
    ...
    try:
        ...
        # For UUID-style job IDs, we need to get the keyword from RabbitMQ or use a default
        keyword = "vulnerability"  # Default fallback
        
        # Try to get job info from RabbitMQ first
        try:
            manager = RabbitMQManager()
            await manager.connect()
            # This would ideally get job metadata, but for now use a sensible default
            await manager.close()
        except (ConnectionError, OSError):
            pass
        
        # If job_id contains readable parts, extract keyword
