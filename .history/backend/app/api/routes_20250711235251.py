...
async def get_job_results(job_id: str):
    ...
    try:
        ...
        try:
            ...
            logger.warning("Connection manager not available, using direct connection")
            manager = None
            try:
                manager = RabbitMQManager()
                await manager.connect()
                result = await manager.get_job_result(job_id)
                # Handle result the same way as above
...
async def get_all_job_results():
    ...
    try:
        try:
            from queue_manager.queue import get_rabbitmq_connection, _job_results_store, _job_metadata_store
            ...
            return {
                ...
                "processing": 0,
                "results": [],
                "error": "Queue manager import failed"
            }
