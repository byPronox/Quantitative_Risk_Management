...
async def get_all_job_results():
    ...
    try:
        ...
        try:
            ...
            all_results = await manager.get_all_job_results()
            ...
            for job in all_results.get("jobs", []):
                if job.get("status") == "completed":
                    formatted_job = {
                        ...
                        "vulnerabilities": job.get("vulnerabilities", [])[:10],  # Limit to 10 for overview
...
@router.get("/nvd/results/{job_id}/detailed")
async def get_detailed_job_results(job_id: str):
    ...
    try:
        ...
        try:
            result = await manager.get_job_result(job_id)
            if result and result.get("status") == "completed":
                # Return all vulnerabilities, not just the first 10
                return {
                    "job_id": job_id,
                    "status": "completed",
                    "keyword": result.get("keyword"),
                    "total_results": result.get("total_results", 0),
                    "vulnerabilities": result.get("vulnerabilities", []),  # All vulnerabilities
                    "processed_via": result.get("processed_via", "queue_consumer"),
                    "processed_at": result.get("timestamp"),
                    "risk_analysis": {
                        "high_severity": len([v for v in result.get("vulnerabilities", []) 
                                            if v.get("cve", {}).get("metrics", {}).get("cvssMetricV2", [{}])[0].get("baseSeverity") == "HIGH"]),
                        "medium_severity": len([v for v in result.get("vulnerabilities", []) 
                                              if v.get("cve", {}).get("metrics", {}).get("cvssMetricV2", [{}])[0].get("baseSeverity") == "MEDIUM"]),
                        "low_severity": len([v for v in result.get("vulnerabilities", []) 
                                           if v.get("cve", {}).get("metrics", {}).get("cvssMetricV2", [{}])[0].get("baseSeverity") == "LOW"])
                    }
                }
            elif result and result.get("status") == "processing":
                return {
        ...
        logger.error("Error getting detailed job results: %s", str(e))
        raise HTTPException(status_code=500, detail=f"Failed to get detailed job results: {str(e)}") from e
