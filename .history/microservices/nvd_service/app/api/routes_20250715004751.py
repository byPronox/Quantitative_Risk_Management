...
async def analyze_cves_with_mongodb(
    ...
    try:
        # Get analysis from NVD service
        result = await nvd_service.analyze_vulnerabilities_by_keywords(keywords, max_results)
        
        # Save results to MongoDB if successful
        if result.get("success") and "jobs" in result:
            await mongodb_service.save_job_results(result["jobs"])
...
@router.post("/mongodb/sync/save-jobs")
async def save_jobs_to_mongodb_endpoint(jobs_data: List[Dict[str, Any]]):
    """Save job results to MongoDB (internal endpoint)"""
    try:
        await mongodb_service.save_job_results(jobs_data)
        return {
            "success": True,
            "message": f"Saved {len(jobs_data)} jobs to MongoDB"
        }
    except Exception as e:
        logger.error("Error saving jobs to MongoDB: %s", str(e))
        raise HTTPException(status_code=500, detail=str(e))
