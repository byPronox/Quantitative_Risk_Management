...
async def bulk_save_to_mongodb():
    """Manually save all completed jobs to MongoDB"""
    try:
        ...
        for job in all_results.get("jobs", []):
            if job.get("status") == "completed":
                job_for_mongodb = {
                    ...
                    "processed_via": "manual_bulk_save",
                    "vulnerabilities": job.get("vulnerabilities", [])
                }
                jobs_to_save.append(job_for_mongodb)
        
        if jobs_to_save:
            await mongodb_service.save_job_results(jobs_to_save)
            return {
                "success": True,
                "message": f"Successfully saved {len(jobs_to_save)} jobs to MongoDB",
        ...
        logger.error(f"Failed to bulk save to MongoDB: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to bulk save to MongoDB: {str(e)}")
