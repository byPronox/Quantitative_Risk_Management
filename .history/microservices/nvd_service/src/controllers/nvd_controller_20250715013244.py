...
async def get_all_mongodb_results():
    ...
    try:
        results = await mongodb_service.get_vulnerabilities()
        return {"results": results, "count": len(results)}
    except Exception as e:
        logger.error(f"Failed to get MongoDB results: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to get MongoDB results: {str(e)}")
...
async def get_mongodb_keyword_reports():
    ...
    try:
        ...
        return {"reports": reports, "total_keywords": len(reports)}
    except Exception as e:
        logger.error(f"Failed to get keyword reports: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to get keyword reports: {str(e)}")

@router.post("/mongodb/save")
async def save_to_mongodb(data: Dict[str, Any]):
    """Save data to MongoDB"""
    try:
        result = await mongodb_service.store_vulnerability(data)
        return {"success": result, "message": "Data saved to MongoDB"}
    except Exception as e:
