...
@router.post("/queue/save_to_mongodb")
async def save_jobs_to_mongodb():
    """Save all in-memory jobs to MongoDB Atlas"""
    try:
        result = queue_service.save_all_jobs_to_mongodb()
        return result
    except Exception as e:
        logger.error(f"Failed to save jobs to MongoDB: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to save jobs to MongoDB: {str(e)}")
