...
class QueueService:
    ...
    def get_all_job_results(self) -> dict:
        ...
        logger.info(f"Found {len(jobs)} in-memory jobs")
        ...
        try:
            ...
            db = client.cveScanner
            
            # Get all jobs from MongoDB
            cursor = db.jobs.find({}).sort("processed_at", -1)
            mongodb_jobs = []
            
            for doc in cursor:
                # Convert ObjectId to string for JSON serialization
                doc["_id"] = str(doc["_id"])
                mongodb_jobs.append(doc)
            ...
            for mongo_job in mongodb_jobs:
                if mongo_job.get("job_id") not in existing_job_ids:
                    ...
                    formatted_job = {
                        ...
                        "processed_at": mongo_job.get("processed_at", mongo_job.get("timestamp", time.time()))
                    }
    ...
    def start_consumer(self):
        """Start a persistent RabbitMQ consumer in a background thread and save existing jobs to MongoDB"""
        # First, save all existing in-memory jobs to MongoDB
        save_result = self.save_all_jobs_to_mongodb()
        logger.info(f"Save to MongoDB result: {save_result}")
        
        if self._consumer_thread is not None and self._consumer_thread.is_alive():
            return {
                "message": "Consumer is already running", 
                "status": "running",
                "mongodb_save": save_result
            }
    ...
    def save_all_jobs_to_mongodb(self):
        """Save all in-memory jobs to MongoDB Atlas when start consumer button is clicked"""
        try:
            jobs_to_save = []
            
            # Get all in-memory jobs that are completed
            for job_id, job in self._jobs.items():
                status = self._job_status.get(job_id, "queued")
                if status == "completed" and job_id in self._job_results:
                    # Create job data in MongoDB format
                    job_for_mongodb = {
                        "job_id": job_id,
                        "keyword": job.get("keyword"),
                        "status": "completed",
                        "total_results": len(self._job_results.get(job_id, [])),
                        "timestamp": job.get("created_at", time.time()),
                        "processed_via": "start_consumer_button",
                        "vulnerabilities": self._job_results.get(job_id, [])
                    }
            ...
            if jobs_to_save:
                ...
                def save_to_mongodb():
                    try:
                        ...
                        loop.close()
                        logger.info(f"Successfully saved {len(jobs_to_save)} jobs to MongoDB via start consumer button")
                        return True
                    except Exception as mongo_error:
                        logger.error(f"Failed to save jobs to MongoDB: {mongo_error}")
                        return False
                ...
                return {
                    "success": success,
                    "message": f"Saved {len(jobs_to_save)} jobs to MongoDB" if success else "Failed to save jobs to MongoDB",
                    "jobs_saved": len(jobs_to_save)
                }
            else:
                return {
                    "success": True,
                    "message": "No completed jobs found to save",
                    "jobs_saved": 0
                }
