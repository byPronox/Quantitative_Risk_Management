...
class QueueService:
    ...
    def _save_all_existing_jobs_to_mongodb(self):
        """Save all existing completed jobs (from memory + MongoDB) to MongoDB Atlas with bulk_save marker"""
        try:
            # First, get all jobs from MongoDB to check what's already there
            existing_job_ids = set()
            all_jobs_from_sources = []
            
            # Get jobs from MongoDB first to check existing IDs
            def get_mongodb_jobs():
                try:
                    ...
                    loop.close()
                    # get_all_jobs returns List[Dict], not dict with "jobs" key
            ...
            # Get all jobs from memory (queue service)
            memory_results = self.get_all_job_results()
            memory_jobs = memory_results.get("jobs", [])
            
            # Combine and process all completed jobs that aren't already saved with bulk_save marker
            jobs_to_save = []
            
            # Process memory jobs
            for job in memory_jobs:
                if (job.get("status") == "completed" and 
                    job.get("vulnerabilities") and 
                    job.get("job_id") not in existing_job_ids):
                    
                    job_for_mongodb = {
                        "job_id": job.get("job_id"),
                        "keyword": job.get("keyword"),
                        "status": "completed",
                        "total_results": job.get("total_results", len(job.get("vulnerabilities", []))),
                        "timestamp": job.get("timestamp", job.get("created_at", time.time())),
                        "processed_via": "bulk_save_on_consumer_start",
                        "vulnerabilities": job.get("vulnerabilities", [])
                    }
                    jobs_to_save.append(job_for_mongodb)
            
            # Also re-save existing MongoDB jobs with bulk_save marker if they don't have it
            for job in mongodb_jobs:
                if (job.get("status") == "completed" and 
                    job.get("vulnerabilities") and 
                    job.get("processed_via") != "bulk_save_on_consumer_start"):
            ...
            if jobs_to_save:
                ...
                def bulk_save_to_mongodb():
                    try:
                        ...
                        loop.close()
                        logger.info(f"Bulk saved {len(jobs_to_save)} existing jobs to MongoDB on consumer start")
                    except Exception as mongo_error:
                        logger.error(f"Failed to bulk save existing jobs to MongoDB: {mongo_error}")
                
                # Save in background thread
                mongodb_thread = threading.Thread(target=bulk_save_to_mongodb, daemon=True)
                mongodb_thread.start()
                
                logger.info(f"Queued {len(jobs_to_save)} jobs (memory + existing MongoDB) for bulk save")
            else:
                logger.info("No new jobs found to bulk save to MongoDB (all existing jobs already marked)")
                
        except Exception as e:
            logger.error(f"Error preparing jobs for MongoDB bulk save: {e}")
    ...
    def start_consumer(self):
        ...
        # First, save all existing completed jobs to MongoDB
