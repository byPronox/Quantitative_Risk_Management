...
class QueueService:
    ...
    def _save_all_existing_jobs_to_mongodb(self):
        """Save all existing completed jobs from /results/all to MongoDB Atlas"""
        try:
            # Get all jobs from get_all_job_results (which includes both memory and any existing data)
            all_results = self.get_all_job_results()
            jobs_data = all_results.get("jobs", [])
            
            jobs_to_save = []
            
            # Convert all completed jobs to MongoDB format
