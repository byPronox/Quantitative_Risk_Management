...
class QueueService:
    ...
    def _save_all_existing_jobs_to_mongodb(self):
        """Save all existing completed jobs in memory to MongoDB Atlas"""
        try:
            jobs_to_save = []
            
            # Get all completed jobs from memory
            for job_id, job in self._jobs.items():
                status = self._job_status.get(job_id, "queued")
                if status == "completed":
                    job_for_mongodb = {
                        ...
                        "keyword": job.get("keyword"),
                        "status": "completed",
                        "total_results": job.get("total_results", len(self._job_results.get(job_id, []))),
                        "timestamp": job.get("timestamp", time.time()),
                        "processed_via": "bulk_save_on_consumer_start",
                        "vulnerabilities": self._job_results.get(job_id, job.get("vulnerabilities", []))
                    }
            ...
            if jobs_to_save:
                ...
                logger.info(f"Queued {len(jobs_to_save)} completed jobs for MongoDB save")
            else:
                logger.info("No completed jobs found to save to MongoDB")
            ...
            try:
                ...
                def callback(ch, method, properties, body):
                    try:
                        ...
                        # --- FETCH VULNERABILITIES USING NVD SERVICE ---
                        vulnerabilities = []
                        total_results = 0
                        try:
                            # Use the NVD service directly instead of calling backend
                            nvd_service = NVDService()
                            
                            async def fetch_vulnerabilities():
                                return await nvd_service.search_vulnerabilities(keywords=keyword)
                            
                            # Run the async function
                            result = asyncio.run(fetch_vulnerabilities())
                            if result and "vulnerabilities" in result:
                                vulnerabilities = result["vulnerabilities"]
                                total_results = result.get("totalResults", len(vulnerabilities))
                            else:
                                logger.warning(f"No vulnerabilities found for keyword: {keyword}")
                        except Exception as e:
