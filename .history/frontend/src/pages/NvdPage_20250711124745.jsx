...
export default function NvdPage() {
  ...
  const fetchQueueStatus = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_NVD_SERVICE_URL || 'http://localhost:8002'}/api/v1/queue/status`);
  ...
  const clearQueue = async () => {
    setQueueLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_NVD_SERVICE_URL || 'http://localhost:8002'}/api/v1/queue/clear`, {
        method: 'DELETE'
      });
