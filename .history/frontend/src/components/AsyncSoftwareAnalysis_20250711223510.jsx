...
export default function AsyncSoftwareAnalysis() {
  ...
  const fetchProcessedResults = async () => {
    try {
      ...
      // Simulate fetching processed results
      // In a real implementation, you'd track job IDs and fetch their results
      const mockProcessedResults = [
        {
          job_id: "processed-apache-" + Date.now(),
          keyword: "apache",
          status: "completed",
          total_results: 2733,
          ...
          vulnerabilities: [
            ...
            },
            {
              cve: {
                id: "CVE-2021-44228",
                published: "2021-12-10T10:15:09.743",
                descriptions: [
                  {
      ...
      mockProcessedResults.forEach(newResult => {
        setProcessedResults(prev => {
          const exists = prev.find(r => r.keyword === newResult.keyword);
          if (!exists) {
  ...
  // Fetch processed results when consumer is running
  useEffect(() => {
