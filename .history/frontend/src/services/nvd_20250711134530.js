...
export async function analyzeSoftwareAsync(softwareList, params = {}) {
  try {
    ...
    // Crear jobs para cada software usando la función addKeywordToQueue existente
    for (const software of softwareList) {
      try {
        const job = await addKeywordToQueue(software);
        jobs.push({
          software: software,
          jobId: job.job_id || `${software}-${Date.now()}`,
          keyword: software
        });
      } catch (error) {
        console.warn(`Failed to queue ${software}:`, error);
        // Crear un job mock si falla
        jobs.push({
          software: software,
          jobId: `${software}-${Date.now()}`,
          keyword: software,
          status: 'queued'
        });
      }
